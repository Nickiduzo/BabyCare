...
public class PinkPig : MonoBehaviour
{
    ...
    {
        transform.DOScale(targetScale, duration * 1.5f).From(initialScale).SetDelay(0.8f);
    }
    private void OnMouseDown()
    {
        if (timeToClick <= 0)
        {
            ...
            soundSystem.PlaySound("isPigSound");
            animator.SetTrigger("isPigAnim");
